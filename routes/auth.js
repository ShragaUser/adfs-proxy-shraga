...
const enrichGenerator = (enrichServiceURL) =>
  trycatch((user, useEnrichId) =>
    axios.post(enrichServiceURL, { useEnrichId, user })
  );

const enrich = enrichment.enrich
  ? enrichGenerator(enrichment.enrich)
  : trycatch(async (x) => ({
      data: x,
    }));
...
const dealWithCallback = async (req, res, next) => {
  ...
  const enrichedUserResp = await enrich(user, req.cookies.useEnrichId);

  //coupled to the enrichment service if its down the we do not return anything.
  if (enrichedUserResp.error)
    return res.status(500).send({ error: enrichedUserResp.error });

  const enrichedUser = enrichedUserResp.result.data;
...
const dealWithSAMLuseADFSCallback = async (req, res, next) => {
  ...
  for (
    ...
    for (let attribute in samlResponseModifier) {
      if (attributes[attributeIndex].getAttribute("Name") === attribute) {
        await getToNodeValueAndAssignNewValue(
          attributes[attributeIndex],
          samlResponseModifier[attribute]
        );
      }
    }
  }
  ...
  const RelayState = req.cookies["RelayState"];
  const htlmResponse = createHTMLResponse(
    newXml,
